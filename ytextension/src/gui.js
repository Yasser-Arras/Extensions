...
export const GUI = (() => {
    ...
    function createMainButton() {
        ...
        button.addEventListener("click", () => {
            ...
            if (toolbar.style.display === "flex") updateToolbarPosition();
        });
    ...
    function updateToolbarPosition() {
        ...
        toolbar.style.top = `${parseInt(mainButton.style.top) + 50}px`;
        toolbar.style.left = `${parseInt(mainButton.style.left) + 20 - toolbar.offsetWidth / 2}px`;
    ...
    function ensureMoreButton() {
        ...
        const img = createIcon(Icons.list);
    ...
    function updateMoreButtonPosition() {
        ...
        const topPx = parseInt(toolbar.style.top, 10) + Math.max(0, Math.floor((toolbar.offsetHeight - 40) / 2));
    ...
    function buildTweaksPanel() {
        ...
        groups.forEach((group) => {
            ...
            tweaksConfig.filter(cfg => cfg.scope === group.key).forEach((cfg) => {
    ...
    function toggleTweaksPanel() {
        if (!tweaksPanel) buildTweaksPanel();
        tweaksPanel.style.display = tweaksPanel.style.display === "none" ? "block" : "none";
    }
...
function makeButton({ label, icon, onClick }) {
    ...
    function destroy() {
        ...
        if (moreButton) moreButton.remove(); moreButton = null;
        if (tweaksPanel) tweaksPanel.remove(); tweaksPanel = null;
        if (mainButton) mainButton.remove(); mainButton = null;
    }
